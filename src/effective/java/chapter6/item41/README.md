...
아이템39에서 마커 애너테이션에 대해 알아봤다. 비슷하게 아무 메서드도 담고 있지 않고, 단지 자신을 구현하는 클래스가 특정 속성을 가짐을 표시해주는 인터페이스를 마커 인터페이스라 한다. 
Serializable 인터페이스가 좋은 예다.
Serializable 인터페이스를 구현한 클래스의 인스턴스는 ObjectOutputStream을 통해 write 할 수 있다(직렬화)는 것을 알 수 있다. 
(자세한 내용은 12장에서 살펴본다)
